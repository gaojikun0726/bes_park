	...
	public static final String encodeObject2Json(Object pObject) {
		...
		if (G4Utils.isEmpty(pObject)) {
			...
			if (pObject instanceof ArrayList) {
				JSONArray jsonArray = (JSONArray) JSONArray.toJSON(pObject);
				jsonString = jsonArray.toString();
			} else {
				JSONObject jsonObject = (JSONObject) JSONObject.toJSON(pObject);
...
//			log.info("序列化后的JSON资料输出:\n" + jsonString);
	...
	public static final String encodeObject2Json(Object pObject, String pFormatString) {
		...
		if (G4Utils.isEmpty(pObject)) {
			...
			if (pObject instanceof ArrayList) {
				net.sf.json.JSONArray jsonArray = net.sf.json.JSONArray.fromObject(pObject, cfg);
				jsonString = jsonArray.toString();
			} else {
...
//	}
	...
	public static Dto parseSingleJson2Dto(String jsonString) {
		...
		net.sf.json.JSONObject jb = net.sf.json.JSONObject.fromObject(jsonString);
		dto = (BaseDto) net.sf.json.JSONObject.toBean(jb, BaseDto.class);
		return dto;
	}
