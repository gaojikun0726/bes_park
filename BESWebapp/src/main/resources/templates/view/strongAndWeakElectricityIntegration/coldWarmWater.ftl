...
<input id = "chilledWaterHiddenInput" type="text" value="" style="display:none"></input>
<div id="chilledWaterDiv">
	<div class="information-model">
		<i class="fa fa-th-list" aria-hidden="true"></i>&nbsp;冷冻水监控信息>>>
	</div>
	<div class="strongAndWeakElectricityIntegrationCommon" style="margin:0px 15px;">
		<table class="layui-hide" id="chilledWaterTable_monitoring" lay-filter="chilledWaterTaskTools"></table>
		<script type="text/html" id="chilledWaterTableOperateTools">
...
<!-- 下方的script作用是序号自增 -->
<script type="text/html" id="chilledWaterTimer">
	{{d.LAY_TABLE_INDEX+1}}
</script>
...
<script type="text/javascript">
	;
	var coldWarmWater = (function($, window, document, undefined) {
		var _ctx = '${ctx}';
		var table;
		var layer;
		var form;
		...
		$(function () {
			queryColdWarmUnit({}, function (result) {
		...
		function init(data) {
			...
			var opTableb = layui.opTable.render({
				elem :'#chilledWaterTable_monitoring',
				id: '#chilledWaterTable_monitoring',
				toolbar: '#toolDemo1',
				...
				width: $('#chilledWaterDiv').width() - 30,
				cols:[[    //表头
					{
						title:"序号",
						templet:'#chilledWaterTimer',
						type:"numbers"
					},
					{
						title:"名称",
					...
					/*{
                        title:"变频器频率设定",//AO 显示数字
                        field:"f_BPQPLSD_alias",
                        align: "center",
                        templet:function (item) {
                            var coolingReturnWaterTemperature = item.f_BPQPLSD;

                            if(coolingReturnWaterTemperature) {
...

					{
						...
						width:200,
						align:"center",
						toolbar:'#chilledWaterTableOperateTools'
					}
				]]
				//  展开的列配置
				, openCols: [
			...
			layui.table.on('toolbar(chilledWaterTaskTools)', function (obj) {
				...
				//水平或垂直排列内容
					...
					opTableb.reload({opOrientation: opTableb.config.opOrientation === "v" ? "h" : "v"});
					setTimeout(function () {
						// 展开第一行
						opTableb.openIndex(0);
						getRealTimeData(function (list) {
							for(var i = 0; i < list.length; i++) {
			...
			$('#chilledWaterDiv').attr('flag', flag);
		}

		// 添加订阅消息
		function addSubMsg() {
            for (let item of sysNames){
                PubSub.subscribe(item, coldWarmWater.subCallback);
            }
		}
		...
		function tableEvent() {
			layui.table.on('tool(chilledWaterTaskTools)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				var data = obj.data //获得当前行数据
				layui.layer.msg('查看操作');
					viewFun(data);
			});
			layui.form.on('switch(lds)', function(data){
			...
			$.ajax({
				...
				success : function(result) {
					...
					if(result.status !== '1'){
						...
						if(val === '0') {
							html = '<input type="checkbox" sys-name="'+ sysName +'" lay-filter="lds" lay-skin="switch" lay-text="开启|关闭" checked>';
						}else if (val === '255') {
							html = '<input type="checkbox" sys-name="'+ sysName +'" lay-filter="lds" lay-skin="switch" lay-text="开启|关闭">';
						}
		...
		function viewFun(data){
			...
			$("#chilledWaterHiddenInput").val(eqName);
			$("#chilledWaterDiv").load(_ctx+'/view/strongAndWeakElectricityIntegration/coldWarmWater/testView',
					{
						"f_NAME": data.f_NAME,
						"f_ID": data.f_ID,
