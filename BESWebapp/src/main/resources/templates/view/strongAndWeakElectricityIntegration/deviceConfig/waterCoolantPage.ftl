...
<div style="height:95%">
    <div id="waterCoolantTable">	</div>
</div>

<!---分页表单----->
<div style="height:5%">
    <form action="${ctx }/view/strongAndWeakElectricityIntegration/deviceConfig/chilledWater/getPaging" id="water_coolant_page_form">
        <!-- 查询条件，用隐藏表单域 -->
        <input type="hidden" value="${keywords! }" name="keywords" />
        <input type="hidden" value="${pageSize! }" id="water_coolant_pageSize" name="water_coolant_pageSize" />
        <input type="hidden" value="${pageNum! }" id="water_coolant_pageNum" name="water_coolant_pageNum" />
        <#assign formId = "water_coolant_page_form"><!-- formId: 分页控件表单ID -->
        <#assign showPageId = "water_coolant_page"><!-- showPageId: ajax异步分页获取的数据需要加载到指定的位置 -->
...
<script type="text/javascript">
    ...
    var waterCoolantUnitPage = (function($, window, document, undefined) {

        //设置操作格式
        var optIconFunction = function(cell, formatterParams){ //plain text value
            var id = cell.getRow().getData().f_ID;
            return "<div class='btn-group '>"
                +"<button class='btn btn-white btn-sm edit' data-id="+ id + " data-toggle='modal' data-target='#water_coolant_modal_edit'><i class='fa fa-pencil' ></i> 编辑</button>"
                +"<button class='btn btn-white btn-sm delete' data-id=" + id + "><i class='fa fa-trash'></i>  删除</button></div>"
        };

        var waterCoolantTable = new Tabulator("#waterCoolantTable", {
            height : "100%",
            layout : "fitColumns",
        ...
        waterCoolantTable.setData('${dataset}'==""?[]:'${dataset}');
