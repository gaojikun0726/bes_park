...
<script src="${ctx}/static/js/cron/timeTaskSyncCron.js" type="text/javascript"></script>
<style type="text/css">
  .line
  {
    position: initial;
    ...
    <div class="easyui-tabs" data-options="fit:true,border:false">
      <div title="秒">
        ...
        <div class="line">
          ...
          秒执行一次</div>
        <div class="line">
          <input type="radio" name="second" id="time_task_sync_sencond_appoint">
          指定</div>
      ...
      <div title="分钟">
        ...
        <div class="line">
          ...
          分钟执行一次</div>
        <div class="line">
          <input type="radio" name="min" id="time_task_sync_min_appoint">
          指定</div>
      ...
      <div title="小时">
        ...
        <div class="line">
          ...
          小时执行一次</div>
        <div class="line">
          <input type="radio" name="hour" id="time_task_sync_hour_appoint">
          指定</div>
      ...
      <div title="日">
        ...
        <div class="line">
          ...
          本月最后一天</div>
        <div class="line">
          <input type="radio" name="day" id="time_task_sync_day_appoint">
          指定</div>
      ...
      <div title="月">
        ...
        <div class="line">
          ...
          月执行一次</div>
        <div class="line">
          <input type="radio" name="mouth" id="time_task_sync_mouth_appoint">
          指定</div>
      ...
      <div title="周">
        ...
        <div class="line">
          ...
          本月最后一个星期<input class="numberspinner" style="width: 60px;" data-options="min:1,max:7"
                         id="week_start_2" value="1"></div>
        <div class="line">
          <input type="radio" name="week" id="time_task_sync_week_appoint">
          指定</div>
    ...
    <fieldset style="border-radius: 3px; height: 220px;">
      ...
      <table style="height: 100px;">
        ...
        <tr>
          ...
          <td>
            <input type="text" name="time_task_sync_second" class="col form-control" value="*" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_min" class="col form-control" value="*" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_hour" class="col form-control" value="*" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_day" class="col form-control" value="*" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_mouth" class="col form-control" value="*" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_week" class="col form-control" value="?" readonly="readonly" />
          </td>
          <td>
            <input type="text" name="time_task_sync_year" class="col form-control" readonly="readonly" />
          </td>
        </tr>
        <tr>
          <td>Cron 表达式:</td>
          <td colspan="6"><input  class="form-control" type="text" name="timeTaskSyncCron" style="width: 100%;" id="timeTaskSyncCron"
            /></td>
          <td>
            <button class="btn btn-md btn-primary" style="width: 5vw" type="button" onclick="time_task_sync_cron_page.btnFan()">反解析到UI</button>
      ...
      <button class="btn btn-md btn-primary" type="button" style="margin-left: 300px;margin-top: 30px;width: 200px;height: 40px;"
              data-dismiss="modal"
              onclick="time_task_sync_cron_page.checkCron()"><strong>选中当前表达式</strong></button>
...
<script type="text/javascript">
  ;
  var time_task_sync_cron_page = (function($, window, document,undefined) {

    $(function () {
      let cronValue = $('#timeTaskSyncCronEdit').val();
      $("#timeTaskSyncCron").val(cronValue);
    })
    //选中当前表达式
    function checkCron() {

      let cron = document.getElementById("timeTaskSyncCron").value;
      timeTaskSync.cron(cron);
    }
    /*killIe*/
    $.parser.parse($("body"));
    ...
    function btnFan() {
      ...
      //获取参数中表达式的值
      var txt = $("#timeTaskSyncCron").val();
      if (txt) {
        var regs = txt.split(' ');
        $("input[name=time_task_sync_second]").val(regs[0]);
        $("input[name=time_task_sync_min]").val(regs[1]);
        $("input[name=time_task_sync_hour]").val(regs[2]);
        $("input[name=time_task_sync_day]").val(regs[3]);
        $("input[name=time_task_sync_mouth]").val(regs[4]);
        $("input[name=time_task_sync_week]").val(regs[5]);
        ...
        if (regs.length > 6) {
          $("input[name=time_task_sync_year]").val(regs[6]);
          initYear(regs[6]);
        }
      }
    }
    ...
    function initObj(strVal, strid) {
      ...
      if (strVal == "*") {
        ...
        $("#" + strid + "_start_0").numberspinner('setValue', ary[0]);
        $("#" + strid + "_end_0").numberspinner('setValue', ary[1]);
      } else if (strVal.split('/').length > 1) {
        ary = strVal.split('/');
        objRadio.eq(2).attr("checked", "checked");
        $("#" + strid + "_start_1").numberspinner('setValue', ary[0]);
        $("#" + strid + "_end_1").numberspinner('setValue', ary[1]);
      } else {
    ...
    function initDay(strVal) {
      ...
      if (strVal == "*") {
        ...
        ary = strVal.split('-');
        objRadio.eq(2).attr("checked", "checked");
        $("#day_start_0").numberspinner('setValue', ary[0]);
        $("#day_end_0").numberspinner('setValue', ary[1]);
      } else if (strVal.split('/').length > 1) {
        ary = strVal.split('/');
        objRadio.eq(3).attr("checked", "checked");
        $("#day_start_1").numberspinner('setValue', ary[0]);
        $("#day_end_1").numberspinner('setValue', ary[1]);
      } else if (strVal.split('W').length > 1) {
        ary = strVal.split('W');
        objRadio.eq(4).attr("checked", "checked");
        $("#day_start_2").numberspinner('setValue', ary[0]);
      } else if (strVal == "L") {
    ...
    function initMonth(strVal) {
      ...
      if (strVal == "*") {
        ...
        ary = strVal.split('-');
        objRadio.eq(2).attr("checked", "checked");
        $("#mouth_start_0").numberspinner('setValue', ary[0]);
        $("#mouth_end_0").numberspinner('setValue', ary[1]);
      } else if (strVal.split('/').length > 1) {
        ary = strVal.split('/');
        objRadio.eq(3).attr("checked", "checked");
        $("#mouth_start_1").numberspinner('setValue', ary[0]);
        $("#mouth_end_1").numberspinner('setValue', ary[1]);
    ...
    function initWeek(strVal) {
      ...
      if (strVal == "*") {
        ...
        ary = strVal.split('/');
        objRadio.eq(2).attr("checked", "checked");
        $("#week_start_0").numberspinner('setValue', ary[0]);
        $("#week_end_0").numberspinner('setValue', ary[1]);
      } else if (strVal.split('-').length > 1) {
        ary = strVal.split('-');
        objRadio.eq(3).attr("checked", "checked");
        $("#week_start_1").numberspinner('setValue', ary[0]);
        $("#week_end_1").numberspinner('setValue', ary[1]);
      } else if (strVal.split('L').length > 1) {
        ary = strVal.split('L');
        objRadio.eq(4).attr("checked", "checked");
        $("#week_start_2").numberspinner('setValue', ary[0]);
      } else {
        objRadio.eq(5).attr("checked", "checked");
        ary = strVal.split(",");
        for (let a = 1; a < 8; a++) {
    ...
    function initYear(strVal) {
      ...
      if (strVal == "*") {
        ...
        ary = strVal.split('-');
        objRadio.eq(2).attr("checked", "checked");
        $("#year_start_0").numberspinner('setValue', ary[0]);
        $("#year_end_0").numberspinner('setValue', ary[1]);
      }
    }
