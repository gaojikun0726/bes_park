...
<div class="leftarea information_left">
	...
	<div id="tree_ryxx" class="Information_area"></div>
</div>
<!-- 单位树模块end -->
...
<div class="information_right">
	<div class="information_size">
		<div class="information-model">
			...
			<!-- 增加按钮 -->
			<a   href="javascript:void(-1);" onclick="ryxxmanage_ryxx.ryxx_show_addmodal()" class="btn btn-primary toLeft"> <i class="fa fa-plus"style="margin-top: 2.5px;margin-left: 2px;" aria-hidden="true"></i>&nbsp;增加
			</a>
			<!-- 搜索框 -->
			<div class="zc_search find">
				  <input type="text" class="find-style"  id="euserkeywords" name="euserkeywords" placeholder="登录账号、用户姓名、公司名称">
				  <button id="queryuserBtn"onclick="ryxxmanage_ryxx.search_euser()"><i class="fa fa-search" aria-hidden="true"></i></button>
			</div>
		</div>
		<div id="ryxxTable" class="Information_area"></div>
	</div>
</div>
<!-- 信息表格模块end -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form" id="addEuser" name="add" class="form-horizontal">
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label">单位编码<span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                        	<input type="text" id="user_add_f_zzjgbh" name="user_add_f_zzjgbh" readonly="readonly" placeholder="请输入单位编号" class="form-control">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-3 control-label">单位名称<span class="text-danger">*</span></label>
                        <div class="col-sm-8">
                        	<input type="text" id="user_add_f_company" readonly="readonly" name="user_add_f_company" placeholder="请输入单位名称" class="form-control">
                        </div>
                    </div>
                    <div class="form-group">
...
<!----编辑用户--->
<div class="modal fade" id="editRyxxFormTable" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <button aria-hidden="true" data-dismiss="modal" class="close" type="button">×</button>
...
<!---添加单位信息结束----->
 ...
 ;
 var ryxxmanage_ryxx =  (function($, window, document, undefined) {
 	var _ctx = '${ctx}';
	var _curRow = null;
	//设置格式
	var optIconFunction = function(cell, formatterParams){ //plain text value
		var user_idyhbh = cell.getRow().getData().f_yhbh;
		return "<div class='btn-group '><button class='btn btn-white btn-sm edit' data-id="+ user_idyhbh + " data-toggle='modal' data-target='#editRyxxFormTable'><i class='fa fa-pencil' ></i> 编辑</button>"
	...
	$("#ryxxTable").tabulator({
		...
		{title:"单位名称", field:"f_company",sorter:"string",align:"left",editor:false,headerSort:false},
		...
		rowClick:function(e, row){
        	_curRow = row;
        	var choiseNode = $('#tree_ryxx').treeview('findNodes', [ _curRow.getData().f_zzjgbh, 'id']);
        	$('#tree_ryxx').treeview('searchByNode', choiseNode);//搜索设置高亮
    	},
	});

	$(window).resize(function(){
		$("#ryxxTable").tabulator("redraw");
	});


	//Clear table on "Empty the table" button click
	$("#clearZzjg").click(function(){
	    $("#ryxxTable").tabulator("clearData");
	});

	$(function () {
	    $.ajax({
	        type: "post",
	        url: _ctx + "/view/ryxxpz/dwxx_tree",
	        dataType: "json",
	        beforeSend: function () {
				showLoad();
			},
	        success: function (result) {
	        	if(result.hasOwnProperty("list")){//判断result返回结果是否包含list
		            ...
		            $('#tree_ryxx').treeview({
		               ...
		               $('#tree_ryxx').treeview('clearSearch');//清除搜索选中高亮
		               		_zzjgbh = nodeData.id;
		               		_zzjgJs = nodeData.js;
		                    $.ajax({
		                	    url: _ctx + "/view/ryxxpz/dwxx_user",
								...
								success: function(result) {
									if(result.hasOwnProperty("list")){
							            $("#ryxxTable").tabulator("setData", result.list);
										}else{
							            $("#ryxxTable").tabulator("setData", []);
										}
					            },
					            complete: function () {
									hiddenLoad();
		            ...
		            $("#tree_ryxx").treeview('collapseAll');
		             var firstNode = $("#tree_ryxx").treeview('findNodes',[result.list[0].id,'id']);
		            $("#tree_ryxx").treeview('expandNode',firstNode);
		        	$("#tree_ryxx").treeview("selectNode", firstNode);//将第一个节点设置为选中状态
	            	}
              	}
	        },
	        complete: function () {
...
$("#euserkeywords").focus(function(){
					  ...
					  if(e.which == "13"){
						  ryxxmanage_ryxx.search_euser();//触发该事件
					    }
					  })
					});
	//添加表单验证
    ...
    //新增保存
	function addform_Ryxx(form) {
	       $.ajax({
		      url: _ctx + "/view/ryxxpz/user_add",
		      type: "post",
		      contentType: "application/json; charset=utf-8",
		      data:JSON.stringify({
		 			...
		 			f_dwxxbh:$("#user_add_f_zzjgbh").val(),
 	...
 	//居中显示（编辑）
 	$('#editRyxxFormTable').on('show.bs.modal', function () {
        $(this).css('display', 'block');
        var modalHeight=$(window).height() / 2 - $('#editRyxxFormTable .modal-dialog').height() / 2;
        $(this).find('.modal-dialog').css({
            'margin-top': modalHeight
        });
	})
    ...
    //删除数据
    $(document).on('click','#ryxxTable button.delete', function () {
        var id=$(this).data("id");
	    swal({
	            title: "您确定要删除吗?",
	            text: "信息删除后将不可恢复!",
		          	  ...
		          	  success: function(data) {
		              	if (data.status == '1') {
			 	        	...
			 	        	title : data.msg,// 展示的标题
		              		...
		              		//在表格中删除该条数据
		              		$("#ryxxTable").tabulator("deleteRow", _curRow);
		              		var getuserTable = $("#ryxxTable").tabulator("getData");
		              		$("#ryxxTable").tabulator("setData", getuserTable);
			            } else{
			                swal(data.msg,"", "error");
			            }
			          },
     ...
     //重置密码
    $(document).on('click','#ryxxTable button.reset', function () {
        var id=$(this).data("id");
	    swal({
	            title: "您确定要重置密码吗?",
	            text: "密码重置后将不可恢复!",
            	...
            	setTimeout(function(){
	            	$.ajax({
			          url: _ctx + "/view/ryxxpz/user_updatepwd",
			          type: "post",
			          data:({
			        			f_yhbh:id,
			        			f_pass:'123456'
  ...
  //关闭模态框清空表单值
    $("#editRyxxFormTable").on('hidden.bs.modal', function (event) {
        $(this).find("input").val("");
  ...
  function edituserForm(form) {
    $.ajax({
      url: _ctx + "/view/ryxxpz/user_update",
      type: "post",
      data: ({
      ...
      success: function(result) {
			if (result.status == '1') {
              $('#editRyxxFormTable').modal('hide');//关闭编辑窗口
              swal({
	 	        	title : result.msg,// 展示的标题
	 	   			text : "",// 内容
	 	   			type: "success",
	         ...
	         $('#ryxxTable').tabulator("updateRow",_curRow, {
			        	 f_yhbh:$("#user_edit_f_yhbh").val(),
			        	 f_name:$("#user_edit_f_name").val(),
			        	 f_zzjgbh:$("#user_edit_f_zzjgbh").val(),
			        	 f_company:$("#user_edit_f_company").val(),
  ...
  //验证在模态框出现前加载
  $("#editRyxxFormTable").on('show.bs.modal', function(event) {
  ...
  //模态框拖动********************
	...
	//*************************************
    $.ajax({
	        url: _ctx + "/view/ryxxpz/loadeditobj",
	        type: "post",
            contentType: "application/json; charset=utf-8",
	        data:JSON.stringify({
	 			f_yhbh:id
  ...
  return {
		//搜索
		 search_euser: function(){
			...
			$("#ryxxTable").tabulator("setData", _ctx+'/view/ryxxpz/user_bykey?euserkeywords='+euserkeywords);
		},
		//验证增加模态框是否弹出
		 ryxx_show_addmodal: function() {
	       var node = $('#tree_ryxx').treeview('getSelected');
