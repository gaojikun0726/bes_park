...
layui.use('layer', function(){
    ...
    function loadRightMenu(){
        $.contextMenu({
            selector: '#design_area_demo .design_div', //右键选择器
            callback: function(key, options) {//点击回调处理
                ...
                if(key === "relative"){
                    ...
                    TempListController.openRelativeWin();
    ...
    //关联模块弹窗
    TempListController.openRelativeWin = function(){
        var dataId = relativeTempListJquery.attr("data-id");
        relativeIndex = layer.open({
            type: 1,
            title:"列表关联",
            area:['400px','200px'],
            content: $('#design_temp_list_relative'), //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
            cancel: function(index, layero){
                TempListController.closeRelativeWin();
                return false;
            },
            success:function () {
                if(dataId){
                    $("#design_temp_list_relative_id").val(dataId);
                    /*$('#settingsTable').attr('id', dataId);*/
                    // todo 根据id查询name
                    $("#design_temp_list_relative_name").val(dataId);
                }
            }
        });
    }
    ...
    //关闭并清空关联模块弹窗
    TempListController.closeRelativeWin = function(){
        layer.close(relativeIndex);
        $("#design_temp_list_relative_id").val("");
        $("#design_temp_list_relative_name").val("");
    }

    //关联点弹窗--关联按钮点击事件
    TempListController.relativeTempList = function () {
        relativeTempListJquery.attr("data-id",$("#design_temp_list_relative_id").val());
        TempListController.closeRelativeWin();
    }
    //关联弹窗--选择点按钮点击事件
    TempListController.chooseModule = function (e) {
        PointTree.$nodeId = $("#design_temp_list_relative_id");
        PointTree.$nodeName = $("#design_temp_list_relative_name");
     ...
     * 关联图片之前校验节点是否满足条件
     */
    TempListController.verifyTemp = function(){
        var sysname = PointTree.$nodeId.val();
        $('#settingsTable').attr('id', sysname);
        var $tempModule = $("#design_area_demo").find(".design_tempImg");
