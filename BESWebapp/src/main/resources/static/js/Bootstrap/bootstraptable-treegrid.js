...
(function ($) {
    ...
    var getChild = function (node, source, field,parentId) {
        var items = $.grep(source, function (item, index) {
    ...
    var getAllChild = function (node, source, field,parentId) {
        var data = [];
        var g=function(child){
            $.each(child, function (i, n) {
                data.push(n);
                var subChild = getChild(n, source, field,parentId);
                if(subChild!=null && subChild.length>0){
                    g(subChild);
                }
            });
        }
        var child = getChild(node, source, field,parentId);
        g(child);
        return data;
    };
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var g=function(child){
                ...
                $.each(child, function (i, n) {
                    ...
                    var subChild = getChild(n, that.data, that.options.treeId,that.options.parentId);
                    if(subChild==null || subChild.length==0){
                        n.isLeaf=true;
                    }
                    rows.push(n);
            ...
            $.each(roots, function (index, item) {
                item.f_js=that.options.treeRootLevel;
                var child = getChild(item, that.data, that.options.treeId,that.options.parentId);
            ...
            if (column.checkbox || column.radio) {
                ...
                value = typeof value === 'undefined' || value === null ?
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = item.f_js == that.options.treeRootLevel ? '' : sprintf('<span style="margin-left: %spx;"></span>', (item.f_js - that.options.treeRootLevel) * 15);
                        var child = $.grep(data, function (d, i) {
                            return d[that.options.parentId] == item[that.options.treeId] && !d.hidden;
                        });
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
            ...
            if(icon.hasClass(that.options.expandIcon)){
                ...
                var child = getAllChild(data[index], data, that.options.treeId,that.options.parentId);
                ...
                var child = getChild(data[index], data, that.options.treeId,that.options.parentId);
                $.each(child, function (i, c) {
                    $.each(that.data, function (index, item) {
                        if (item[that.options.treeId] == c[that.options.treeId]) {
                            item.hidden = false;
    ...
    BootstrapTable.prototype.expandAllTree = function ()
    {
        ...
        $.each(roots, function (index, item) {
            var child = getAllChild(item, that.options.data, that.options.treeId,that.options.parentId);
            $.each(child, function (i, n) {
                n.hidden=false;
            });
        });
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        parentId: "parentId", //父节点字段ID
        treeId: "id",
        treeRootLevel: 1,//根节点序号
        treeCollapseAll: true,//是否全部展开，默认不展开
        collapseIcon: "glyphicon glyphicon-plus-sign",//折叠样式
        expandIcon: "glyphicon glyphicon-minus-sign",//展开样式
        leafIcon:"glyphicon glyphicon-stop"//叶子节点样式
    });
