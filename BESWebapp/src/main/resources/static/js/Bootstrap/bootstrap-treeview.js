...
;(function ($, window, document, undefined) {
	...
	_default.settings = {
		...
		expandIcon: 'glyphicon glyphicon-chevron-right',//扩展图标
		collapseIcon: 'glyphicon glyphicon-chevron-down',//折叠图标
		loadingIcon: 'glyphicon glyphicon-hourglass',//加载图标
		emptyIcon: '',//清除图标
		nodeIcon: '',
		selectedIcon: '',
		...
		onhoverColor: '#1f9fe87d',
		selectedColor: '#d1e3f9',
		selectedBackColor: '#255274',
		searchResultColor: '#22b4ea',
		...
		staticLoad: false // 静态加载（默认动态加载dom）
	};
	...
	var Tree = function (element, options) {
		...
		return {
			...
			updateNodeOld: $.proxy(this.updateNodeOld, this),
			...
			searchByNode: $.proxy(this.searchByNode, this),
			clearSearch: $.proxy(this.clearSearch, this),
			
			getChildsArrayByNode: $.proxy(this.getChildsArrayByNode, this),
			getChildsMapByNode: $.proxy(this.getChildsMapByNode, this)
		};
	};
	...
	*/
	...
	Tree.prototype._setExpanded = function (node, state, options) {
		...
		if (state && (node.nodes || node.isChildNodeExist === '1')) {
			...
			var lazyLoadFun = this._options.lazyLoad;

			if (typeof lazyLoadFun === 'function') {

				if (node.isChildNodeExist === '1' && !node.nodes) {
					lazyLoadFun( nodes =>{
	...
	Tree.prototype._renderStatic = function () {
		if (!this._initialized) {
		...
		var htmlText = [];
		$.each(this._orderedNodes, $.proxy(function (id, node) {
			this._renderNodeStatic(node, previousNode, htmlText);
			previousNode = node;
		}, this));

		this.$wrapper.empty();
		this.$wrapper.prepend(htmlText.join(''));

		for (var i = 0; i < this._orderedNodes.length; i++) {

			var node = this._orderedNodes[i];
			var id = node.id;
			node.$el = $(`#${id}`);

			// Set various node states

			if (node.isChildNodeExist === '1'){
				node.state.expanded = true;
			}
	...
	Tree.prototype._renderAddNode = function (node, previousNode) {
		if (!node) return;

		node.$el = this._template.node.clone();

		if(previousNode.nodes && previousNode.nodes.length >= 2) {
			var childrenNodes = previousNode.nodes;
			this._addNodeAfter(childrenNodes[childrenNodes.length - 2])
			.$el.after(node.$el);
		}else {
			previousNode.$el.after(node.$el);
		}

		node.$el.addClass('node-' + this._elementId);
		...
		node.$el
			.append(
				node.nodes || (node.lazyLoad || node.isChildNodeExist === '1') ? this._template.icon.expand.clone() : this._template.icon.empty.clone()
			);
	...
	// 在什么之后添加节点
	Tree.prototype._addNodeAfter = function (node) {
		...
		var nodeLast = nodes[nodes.length -1];

		return this._addNodeAfter(nodeLast);

	}

	Tree.prototype._renderUpdateNode = function (node) {
		...
		$.extend(this._nodes[node.nodeId], node);
	...
	Tree.prototype._renderNodeStatic = function (node, previousNode, htmlText) {
		if (!node) return;
		...
		if (node.dataAttr) {
			$.each(node.dataAttr, function (key, value) {
				customAttr.push(`data-${key}="${value}"`);
			});
		}
		...
		for (var i = 0; i < (node.level - 1); i++) {
			indent.push(`<span class="indent"></span>`);
		}

		var icon = '';

		if (node.nodes || node.isChildNodeExist === '1')
		{
			icon = `<span class="icon expand-icon"></span>`;
		}else
		{
		...
		if (this._options.checkboxFirst) {
			checkbox.push(this._addCheckboxStatic(node));
			checkbox.push(this._addIconStatic(node));
			checkbox.push(this._addImageStatic(node));
		} else {
			checkbox.push(this._addIconStatic(node));
			checkbox.push(this._addImageStatic(node));
			checkbox.push(this._addCheckboxStatic(node));
		...
		if (this._options.wrapNodeText) {
			text = `<span class="text">${node.text}</span>`;
		} else {
			text = node.text;
		}
		...
		if (this._options.showTags && node.tags) {
			$.each(node.tags, $.proxy(function addTag(id, tag) {
				tags.push(`<span class="${(typeof tag === 'object' ? tag.class : undefined) || node.tagsClass || this._options.tagsClass}">${(typeof tag === 'object' ? tag.text : undefined) || tag}</span>`);
			}, this));
		}

		htmlText.push(`<li class="list-group-item node-${this._elementId}" id="${node.id}" ${customAttr.join(' ')} data-nodeId="${node.nodeId}" ${title}>${indent.join('')}${icon}${checkbox.join('')}${text}${tags.join('')}</li>`);
	...
	// Add checkable icon
	Tree.prototype._addCheckboxStatic = function (node) {
		if (this._options.showCheckbox && (node.hideCheckbox === undefined || node.hideCheckbox === false)) {
			return `<span class="icon check-icon"></span>`;
		}
	...
	// Add node icon
	Tree.prototype._addIconStatic = function (node) {
		if (this._options.showIcon && !(this._options.showImage && node.image)) {
			return `<span class="icon node-icon" class="${node.icon || this._options.nodeIcon}"></span>`;
		}
	...
	Tree.prototype._addImageStatic = function (node) {
		if (this._options.showImage && node.image) {
			return `<span class="image node-image" style="background-image: url(${node.image}); height: 35px; width: 35px"></span>`;
		}
	...
	Tree.prototype.addNode = function (nodes, parentNode, index, options) {
		...
		this._setInitialStates({nodes: this._tree}, 0)
			.done($.proxy(function () {
				...
				// this._render();
				if (this._options.staticLoad) {
					this._renderAddNode(nodes[0], parentNode);
					this._renderUpdateNode(parentNode);
				}else {
					this._render();
				}
			}, this));
	...
	*/
	Tree.prototype.removeNode = function (nodes, options) {
		...
		if (!this._options.staticLoad) {
			...
			this._setInitialStates({nodes: this._tree}, 0)
				.done($.proxy(function () {
					this.updateDataNodeId(targetNodes);
				}, this));
	...
	Tree.prototype.updateDataNodeId = function (targetNodes, parentNodeId) {
		...
		for (var i = 0; i < targetNodes.length; i++) {
			...
			if (parentNodeId) {
				...
				nodeId = node.nodeId = node.nodeId.replace(/.\d+$/, '.' + i);
			}

			node.$el.attr('data-nodeId', nodeId);

			var childrenNode = node.nodes;

			if (childrenNode && childrenNode.length > 0) {
				this.updateDataNodeId(childrenNode, nodeId);
			}
		}
	...
	/**
		...
		@param {optional Object} options
	*/
	Tree.prototype.updateNodeOld = function (node, newNode, options) {
		if (node instanceof Array) {
			node = node[0];
		}
...
//=============================my
	...
	@return {Array} nodes - Matching nodes
*/
Tree.prototype.searchByNode = function (pattern, options) {
	options = $.extend({}, _default.searchOptions, options);
	...
	*/
	...
	/*
	 * 获取当前节点下所有子节点,返回Array
	 * node : 需要查询的node节点
	 * 注意：返回结果集中不包含当前node节点本身
	 */
	Tree.prototype.getChildsArrayByNode = function (node) {
		if(node == null || node == ""){
			return new Array;
		}
		var nodeObj =  this._findNodes(node.id, 'id');
		if(nodeObj instanceof Array ){
			nodeObj = nodeObj[0];
		}
		var arrList = new Array();
		this._findChildArray(nodeObj, arrList);
		return arrList;
	};
	
	Tree.prototype._findChildArray = function (node, arrList) {
		if(node == null || node == ""){
			return;
		}
		...
		if(nodeChilds != null && nodeChilds.length > 0){
			for(var i = 0; i<nodeChilds.length; i++){
				arrList.push(nodeChilds[i].id);
				if(nodeChilds[i].hasOwnProperty("nodes")){
					this._findChildArray(nodeChilds[i], arrList);
				}
			}
		}
	};
	 ...
	 * 注意：返回结果集中不包含当前node节点本身
	 */
	Tree.prototype.getChildsMapByNode = function (node) {
		if(node == null || node == ""){
			return null;
		}
		var attrMap = new Map();
		this._findChildMap(node, attrMap);
		return attrMap;
	};
	
	Tree.prototype._findChildMap = function (node, attrMap) {
		if(node == null || node == ""){
			return;
		}
		var nodeChilds = node.nodes;
		if(nodeChilds != null && nodeChilds.length > 0){
			for(var i = 0; i<nodeChilds.length; i++){
				attrMap.put(nodeChilds[i].id, nodeChilds[i].text);
				if(nodeChilds[i].hasOwnProperty("nodes")){
					this._findChildMap(nodeChilds[i], attrMap);
				}
			}
		}
	};
