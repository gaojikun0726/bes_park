...
;(function($) {
	...
	function buildHTML(tableObj, opts) {
		var wrapper = $('<div class="fixed-thead" style="height:'+opts.thead+'"></div>'),
			bodyWrap = $('<div class="fixed-thead-body" style="height:'+opts.body+'"></div>');
	...
	function buildHTML_side(configs) {
		...
		if (wrap.length == 0) {
			wrap = $('<div class="fixed-thead-side" style="height:'+configs.opts.side+'"></div>');
