...
import com.efounder.JEnterprise.zhdg.dao.WarntypeDao;
...
public class WarntypeServiceImpl implements WarntypeService {
    @Autowired
    private WarntypeDao dao;

    @Override
    public PageInfo<WarntypeEntity> getSearch(String keywords, Integer pageNum) {
        if (pageNum == null)
            pageNum = 1;
        PageHelper.startPage(pageNum, Constants.PAGE_SIZE);
        List<WarntypeEntity> list = dao.getSearch(keywords);
        PageInfo<WarntypeEntity> page = new PageInfo<WarntypeEntity>(list);
        return page;
    }
    ...
    */
    ...
    public ISSPReturnObject getAllWarnType() {
        ISSPReturnObject returnObject = new ISSPReturnObject();
        try {
            List<WarntypeEntity> list = dao.getSearch("");
            returnObject.setStatus("1");//1.成功
