 ...
 * @author tianjiangwei
 */
@RequestMapping(value = "/view/userpostgnqxquery/")
@Controller
public class ESUserPostGnqxQueryController {

	private static final Logger log = LoggerFactory.getLogger(ESUserPostGnqxQueryController.class);

	@Autowired
	private ESUserPostGnqxQueryService esUserPostGnqxQueryService;
	@Autowired
	private ESUserGnqxQueryService esUserGnqxQueryService;
	
	/**
	 * @Description 初始化加载岗位功能权限查询主页面
	 * @param model
	 * @return
	 */
	@RequestMapping(value = "user_postgnqxquery", method = RequestMethod.GET)
	public String getUserGnqxQueryPage() {    
		log.info("#ESUserGnqxQueryController初始化加载岗位功能权限查询主页面");
		return "/isspview/usermanage/authorityquery/user_postgnqxquery";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryPostGnzdData", method = RequestMethod.POST)
	@ResponseBody
	public ISSPReturnObject queryGnzdData(@RequestParam(value = "f_gwguid") String f_gwguid) {
		
		log.info("#获取功能权限数据");
		ISSPReturnObject returnObject = new ISSPReturnObject();
		List<ESGnzdManage> query_list = new ArrayList<ESGnzdManage>();
		query_list = esUserPostGnqxQueryService.queryPostGnzdData(f_gwguid);//根据岗位编号过滤功能字典
