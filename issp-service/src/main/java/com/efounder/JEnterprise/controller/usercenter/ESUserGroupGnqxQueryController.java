 ...
 * @author tianjiangwei
 */
@RequestMapping(value = "/view/usergroupgnqxquery/")
@Controller
public class ESUserGroupGnqxQueryController {

	private static final Logger log = LoggerFactory.getLogger(ESUserGroupGnqxQueryController.class);

	@Autowired
	private ESUserGroupGnqxQueryService esUserGroupGnqxQueryService;
	@Autowired
	 ...
	 * @return
	 */
	@RequestMapping(value = "user_groupgnqxquery", method = RequestMethod.GET)
	public String getUserGnqxQueryPage() {    
		log.info("#ESUserGnqxQueryController初始化加载用户组功能权限查询主页面");
		return "/isspview/usermanage/authorityquery/user_groupgnqxquery";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryGroupGnzdData", method = RequestMethod.POST)
	@ResponseBody
	public ISSPReturnObject queryGnzdData(@RequestParam(value = "f_zbh") String f_zbh) {
		
		log.info("#获取功能权限数据");
		ISSPReturnObject returnObject = new ISSPReturnObject();
		List<ESGnzdManage> query_list = new ArrayList<ESGnzdManage>();
		query_list = esUserGroupGnqxQueryService.queryGroupGnzdData(f_zbh);//根据用户组编号过滤功能字典
			...
			for(int i = 0;i<tree.size();i++){
				for(int j = 0;j<query_list.size();j++){
					if((tree.get(i).getF_guid()).equals(query_list.get(j).getF_gnzdid())){
						...
						tree.get(i).setF_sq("1");//用户组功能权限表中存在的即为已授权的，否则是未授权的
						break;				
						}else{
							tree.get(i).setF_sh("0");
							tree.get(i).setF_sq("0");
