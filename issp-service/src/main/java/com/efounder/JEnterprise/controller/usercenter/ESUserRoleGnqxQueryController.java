...
/**
 * @Description 用户角色功能权限
 * @author tianjiangwei
 */
@RequestMapping(value = "/view/userrolegnqxquery/")
@Controller
public class ESUserRoleGnqxQueryController {

	private static final Logger log = LoggerFactory.getLogger(ESUserRoleGnqxQueryController.class);

	@Autowired
	private ESUserRoleGnqxQueryService esUserRoleGnqxQueryService;
	@Autowired
	 ...
	 * @return
	 */
	@RequestMapping(value = "user_rolegnqxquery", method = RequestMethod.GET)
	public String getUserGnqxQueryPage() {    
		log.info("#ESUserGnqxQueryController初始化加载用户功能权限查询主页面");
		return "/isspview/usermanage/authorityquery/user_rolegnqxquery";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryRoleGnzdData", method = RequestMethod.POST)
	@ResponseBody
	public ISSPReturnObject queryGnzdData(@RequestParam(value = "f_rolebh") String f_rolebh) {
/*		log.info("#获取角色功能权限数据");
		...
		query_list = esUserRoleGnqxQueryService.queryRoleGnzdData(f_rolebh);//根据用户编号过滤功能字典
		list = esUserRoleGnqxQueryService.loadAllRoleGnzdData();//查询所有的功能字典
		list1 = esUserRoleGnqxQueryService.loadAllRoleGnzdDatasc();//查询所有的功能字典
		//list.addAll(list1); 
		for(int i = 0;i<list.size();i++){
			for(int j = 0;j<query_list.size();j++){
				if((list.get(i).getF_guid()).equals(query_list.get(j).getF_gnzdid())){
					...
					list.get(i).setF_sq("1");//用户角色功能权限表中存在的即为已授权的，否则是未授权的
					break;				
					}else{
