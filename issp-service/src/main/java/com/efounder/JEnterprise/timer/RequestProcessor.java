...
//        	"DefaultRequestProcessor");
    	...
    	if (value == null || value.trim().length() == 0){
    		...
    		if(throughput == 50){
    			throughput = Integer.parseInt(value);
    		}
    	}
...
//    }
     ...
     */
    public final class Task extends com.efounder.JEnterprise.timer.Task implements Cancellable {
        private Item item;
            ...
            if (delay == 0) { // Place it to pending queue immediatelly
                ...
                starterThread.schedule( new TimerTask() {
                    public void run() { // enqueue the created
                        // guard code for #33302. If it ever happens again
    ...
    void enqueue (Item item) {
        synchronized (processorLock) {
            ...
            if (running < throughput) {
            	...
            	System.out.println(this.name+">>>>超出最大任务吞吐量("+String.valueOf(this.throughput)+")。");
            }
        }
    }
...
//------------------------------------------------------------------------------
    ...
    private static class Processor extends Thread {
        ...
        static void put (Processor proc, String last) {
            synchronized (pool) {
                proc.setName("Inactive RequestProcessor thread [Was:" +
                    proc.getName() + "/" + last + "]"); // NOI18N
                proc.idle = true;
                pool.push(proc);
            }
        }
...
//                if (loggable) em.log (ErrorManager.INFORMATIONAL, "Begining work " + getName ()); // NOI18N
                ...
                while((todo = current.askForWork(this, debug)) != null) {
                    // if(todo != null) {
                        ...
                        try {
//                            if (loggable) logger ().log ("  Executing " + todo); // NOI18N
                            todo.run ();
//                            if (loggable) logger ().log ("  Execution finished in" + getName ()); // NOI18N
                            debug = todo.debug();
