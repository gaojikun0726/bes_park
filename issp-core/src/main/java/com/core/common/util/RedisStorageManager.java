...
public class RedisStorageManager {
	...
	boolean isDisableCache = !"1".equals(enable);
	...
	public static RedisStorageManager getDefault() {
	...
	public void setJsonField(String key, String field, String value) {
		JSONObject obj = JSON.parseObject(stringRedisTemplate
	...
	public <T> void setMap(String key, T obj, long time) {
		Map<String, String> map = (Map<String, String>) JsonMapper.parseObject(
				obj, Map.class);
		redisTemplate.opsForHash().putAll(key, map);
		if (time > 0) {
			stringRedisTemplate.expire(key, time, TimeUnit.SECONDS);
